# TotalSpineSeg Workflow

TotalSpineSeg Workflow is a pipeline for segmentation of the spine using TotalSpineSeg [^1].

## Example

The configuration file [example.yaml](./example.yaml) provides an example configuration file for segmenting all images in the example dataset. The pipeline can be executed with:

```bash
uv run python ./workflow.py --config ./segment_example_dataset.yaml --totalspineseg-data-dir totalspineseg-data --device cuda --no-totalspineseg-quiet
```
